...
(define-abstraction-dependent-metafunctions
  (local
  ...
  (concrete
   ...
   ;; FIXME: Doesn't work for semantic non-tail-calls
   (define-metafunction DCPS-machine
     ...
     [(pop/rebind-necessarily ulab clam u     v ρ Ξ D)
      ,(match-let* ([(call-label-info refnonheap ulam-depth) (hash-ref label-ht (term ulab))]
                    [S? (not (eq? heap-ref (hash-ref refnonheap (term u))))]
   ...
   (define-metafunction DCPS-machine
     [(color-cont (name ς ((clo (λ ulab (u ... k) exp) ρ) (d ..._i) conf-kont σ Ξ D))
                  n_depth ρ_1 σ_1 Ξ_1 D_1 any)
      ((env-extend ρ_1 k ℓ)
       ,(combine-stores (term σ_1) (term σ_new) #f (box #f))
       (ξ_top ξ_rest ...)
       (update-display D_1 n_depth ξ_top))
...
(define (combine-stores σ₁ σ₂ allow-GC? loss?)
  (let/ec return
    (for/fold ([ret σ₁]) ([(k vs) (in-dict σ₂)])
      ...
      (cond [(and allow-GC? (unbox loss?)) (return #f)]
            [else (dict-set ret k (list kvs*))]))))

;; if pushing values that reference the stack at the same nesting depth, then join!
(define-metafunction DCPS-machine
  [(color-user ς bool_extend n_depth (u ..._i) (d ..._i) ρ σ (ξ_top ξ ...) D)
   ,(let ()
      (define loss? (box #f)) ;; INVARIANT: never #t in concrete semantics.
      (let/ec return
        (let recur ([GCed? #f] [σ (term σ)] [D (term D)])
          (define (combine-stores* σ₁ σ₂)
            (define maybe-σ₁₂ (combine-stores σ₁ σ₂ (not GCed?) loss?))
            (cond [maybe-σ₁₂ maybe-σ₁₂]
                  ;; needs GC
                  [else (define-values (σlocs Ξvars) (ℛ (term ς)))
                        (return (recur #t
                                       (restrict-σ σ σlocs)
                                       (restrict-D D Ξvars)))]))
          ...
          (define ξ_next
            (let ([ξ* (cond [extend? (combine-frames (term ξ_top) (term ((u d) ...)))]
                            [else (term ((u d) ...))])])
              (combine-stores* ξ* (term (display-lookup ,D n_depth)))))
          (define Ξ_next
          ...
          (term ((env-extend* ρ (u ...) ,ℓs)
                 ...
                 ,Ξ_next
                 (update-display ,D n_depth ,ξ_next)
                 ,(unbox σlocs))))))])
...
(define-metafunction DCPS-machine
  [(Au ulab u ρ σ D)
   ,(match-let* ([(call-label-info refnonheap ulam-depth) (hash-ref label-ht (term ulab))]
                 [S? (hash-ref refnonheap (term u))] ;; #t iff stack ref.
                 [depth (get-depth coloring (term u))])
      (cond [depth
...
|#
    ;; user apply (entry)
    [--> (name ς ((clo (λ ulab (u ..._i k) exp) ρ) (d ..._i) conf-kont σ Ξ D))
         (exp ρ_2 σ_new2 Ξ_2 D_2)
         (where n_depth ,(hash-ref label-ht (term ulab)))
         (where (ρ_1 σ_new Ξ_1 D_1 (name σlocs (any ...)))
                (color-user ς #f n_depth (u ...) (d ...) ρ σ Ξ D))
         (where (ρ_2 σ_new2 Ξ_2 D_2)
                (color-cont ς n_depth ρ_1 σ_new Ξ_1 D_1 σlocs))]
...
(define (GCFA2 e #:tr [tr translate] #:inject [inj inject])
  ...
  (let whole-analysis ()
    ...
    (let analyze ()
      (match (set-grab Work)
        [#f ;; no more work. Can we do better with our new escape information?
         (define change? (recolor! coloring label-ht Escaping))
         (cond [change? (printf "Better escape information. Bootstrapping...~%~%")
                        ;; Answer: Yup. Do it again now that we recolored defs and refs.
                        (whole-analysis)]
               ;; Answer: Nope.
               [else (list Summary Final Escaping)])]
        [(list ς̃₁ ς̃₂)
         ...
         ((term-match/single DCPS-machine
            ...
            [ς-exit-shape
             (let ()
               ...
               (match-define (call-label-info refnonheap ulam-depth)
                             (hash-ref label-ht lab))
               (printf "Exit-")
               (cond [(dict-ref refnonheap (term k) #t)
                      ;; TODO: handle exn colored k.
                      (printf "Ret~%")
                      (cond [(equal? ς̃₁ I)
                             (printf "Ret Final ς̃₂~%")
                             (escape-analysis ς̃₂)
...
;; Call GCFA2 if using local semantics, otherwise call apply-reduction-relation*
(define-syntax (run/analyze stx)
  (if use-concrete-metafunctions?
      (syntax-case stx ()
        [(_ t) #'()
         (syntax/loc stx (apply-reduction-relation* R-DCPS-machine (inject t)))])
      (syntax-case stx ()
        [(_ t) (syntax/loc stx (GCFA t))])))