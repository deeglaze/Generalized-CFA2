...
(define (in-stack? x) (dict-ref coloring x #f))
...
(define-metafunction Istack
  ...
  [(ℓ-lookup σ ξ #t (Stack natural_s))                (,(list-ref (term ξ) (term natural_s)))]
...
(define R-Istack
  (reduction-relation Istack
    ...
    [--> (d ρ σ Ξ D scope-depth_current κ) (v σ Ξ D κ)
         (where (name ref (ann-ref x bool scope-depth)) (untail d))
         (where (v_pre ... v v_post ...) (lookup ρ σ D ref))]
